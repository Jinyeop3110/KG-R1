...
def get_target_answers(ground_truth):
    """Extract target answers from ground_truth, handling both 'target' and 'target_text' formats."""
    if 'target' in ground_truth:
        return ground_truth['target']
    elif 'target_text' in ground_truth:
        return ground_truth['target_text']
    else:
        raise KeyError(f"Ground truth must contain either 'target' or 'target_text' field. Found keys: {list(ground_truth.keys())}")
...
def compute_score_em(solution_str, ground_truth, method='strict', structure_format_score=0, final_format_score=0, retrieval_score=0, format_score=0, score=1.):
    ...
    target_answers = get_target_answers(ground_truth)
    retrieval_correct = False
    if is_valid_format:
        retrieval_correct = is_retrieval_correct(solution_str, target_answers)
    answer = extract_solution(solution_str=solution_str)
    do_print = random.randint(1, 64) == 1
    
    if do_print:
        print(f"--------------------------------")
        print(f"Golden answers: {target_answers}")
