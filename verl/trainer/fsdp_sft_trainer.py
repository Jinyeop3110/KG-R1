    ...
    def _compute_loss_and_backward(self, batch, do_backward=True):
        ...
        with context, torch.autocast(device_type=self.device_name, dtype=torch.bfloat16):
            if not use_sp:
                ...
                output = self.fsdp_model(input_ids=input_ids_int, attention_mask=attention_mask, position_ids=position_ids, use_cache=False)
                ...
                # Forward pass
                # Ensure input_ids remain as integers (not affected by autocast)
                input_ids_rmpad_sliced_int = input_ids_rmpad_sliced.long()
                
                output = self.fsdp_model(
                    input_ids=input_ids_rmpad_sliced_int,
