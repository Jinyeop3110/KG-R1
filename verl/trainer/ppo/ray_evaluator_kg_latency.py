...
"""
Ray-based KG Evaluator with Ultra-Fast Latency Mode for Performance Measurement.

This evaluator is optimized for maximum generation speed with comprehensive timing metrics.
It removes all expensive overhead operations while providing detailed throughput analysis.

Key features:
- Ultra-fast latency mode (minimal overhead)
- Comprehensive generation timing capture
- Token throughput metrics (samples/sec, tokens/sec)
- Multi-turn KG generation timing breakdown
- Compatible with existing evaluation pipeline
- Does not interfere with training or normal evaluation
"""
...
class RayKGEvaluatorLatency(RayPPOTrainer):
    """
    Ultra-Fast KG Evaluator optimized for latency measurement and timing analysis.
    
    Key differences from standard evaluator:
    - Ultra-fast latency mode with minimal overhead
    - Comprehensive generation timing capture 
    - Token throughput metrics (samples/sec, tokens/sec)
    - Multi-turn timing breakdown for KG reasoning
    - Removes expensive decode/logging operations
    - Only Pass@1 metrics (fastest evaluation)
    - Does not interfere with training or normal evaluation
    """
    ...
    def __init__(
        ...
        val_reward_fn=None,
        eval_samples: int = 0,
        save_detailed_results: bool = False,  # Ultra-fast mode: disabled
        ray_worker_group_cls=None,
        latency_mode: str = "ultra_fast",  # Force ultra-fast mode
        **kwargs
    ):
        ...
        # Ultra-fast latency mode configuration
        self.latency_mode = latency_mode
        self.n_rollout_eval = n_rollout_eval
        
        # Ultra-fast mode: only Pass@1 for maximum speed
        if latency_mode == "ultra_fast":
            self.k_values = [1]  # Only most important metric
            self.save_detailed_results = False  # Disable expensive I/O
            self.enable_debug_logging = False  # Disable tokenizer.decode() calls
            self.minimal_metrics = True  # Only essential metrics
        else:
            self.k_values = k_values or [1, 3, 5, 8]
            self.save_detailed_results = save_detailed_results
            self.enable_debug_logging = True
            self.minimal_metrics = False
        ...
        # Ultra-fast mode: minimal initialization logging
        if not self.minimal_metrics:
            print(f"KG Latency Evaluator initialized with n_rollout_eval={self.n_rollout_eval}, k_values={self.k_values}")
            print(f"Latency mode: {self.latency_mode}, KG Search enabled: {self.use_search_generation}")
        else:
            print(f"[KG-LATENCY] Ultra-fast mode: Pass@{self.k_values[0]} only, minimal overhead")
    ...
    def evaluate_dataset(self, dataset_name: str = "test") -> Dict[str, Any]:
        ...
        # Print ultra-fast timing metrics if available
        if self.latency_mode == "ultra_fast":
            print(f"\n{'='*60}")
            print(f"KG-R1 LATENCY TIMING METRICS")
            print(f"{'='*60}")
            
            if 'total_generation_time' in final_metrics:
                print(f"Total Generation Time: {final_metrics['total_generation_time']:.2f}s")
            if 'avg_generation_time' in final_metrics:
                print(f"Average Generation Time: {final_metrics['avg_generation_time']:.2f}s per batch")
            if 'total_total_tokens' in final_metrics:
                print(f"Total Tokens: {final_metrics['total_total_tokens']:.0f}")
            if 'total_output_tokens' in final_metrics:
                print(f"Total Output Tokens: {final_metrics['total_output_tokens']:.0f}")
            if 'avg_samples_per_sec' in final_metrics:
                print(f"Average Samples/sec: {final_metrics['avg_samples_per_sec']:.2f}")
            if 'avg_tokens_per_sec' in final_metrics:
                print(f"Average Tokens/sec: {final_metrics['avg_tokens_per_sec']:.2f}")
            if 'avg_output_tokens_per_sec' in final_metrics:
                print(f"Average Output Tokens/sec: {final_metrics['avg_output_tokens_per_sec']:.2f}")
    ...
    def _evaluate_batch(self, test_data: dict) -> Dict[str, Any]:
        ...
        # Generate responses with comprehensive timing capture
        import time
        generation_start_time = time.time()
        
        if not self.use_search_generation:
            # Standard generation timing
            if not self.async_rollout_mode:
                test_output_gen_batch_padded = self.actor_rollout_wg.generate_sequences(test_gen_batch_padded)
            else:
                test_output_gen_batch_padded = self.async_rollout_manager.generate_sequences(test_gen_batch_padded)
        else:
            # KG search generation timing (multi-turn)
            ...
            # Ultra-fast mode: skip expensive sample detail logging
            if not self.minimal_metrics:
        ...
        # Compute token counts and throughput metrics for timing analysis
        batch_size = len(test_gen_batch_padded.batch) if hasattr(test_gen_batch_padded, 'batch') else len(test_gen_batch_padded)
        
        # Count input tokens (prompts)
        input_token_count = 0
        if 'input_ids' in test_gen_batch_padded.batch:
            input_ids = test_gen_batch_padded.batch['input_ids']
        ...
        if 'responses' in test_output_gen_batch_padded.batch:
            responses = test_output_gen_batch_padded.batch['responses']
        ...
        # Store timing metrics for later aggregation
        timing_metrics = {
        ...
        # Ultra-fast mode: minimal console output
        if self.minimal_metrics:
            print(f"[KG-R1-TIMING] Generation: {generation_time:.2f}s, {samples_per_sec:.1f} samples/sec, {tokens_per_sec:.0f} tokens/sec")
        ...
        # Ultra-fast mode: skip expensive debug logging
        if not self.minimal_metrics:
        ...
        # Ultra-fast mode: skip expensive batch preview with tokenizer.decode()
        if not self.minimal_metrics:
        ...
        # Only do expensive tokenizer.decode() operations if not in minimal metrics mode
        if not self.minimal_metrics:
        ...
        # Add timing metrics to batch results
        batch_metrics.update({
            'inputs': sample_inputs,
            'outputs': sample_outputs,
            'scores': sample_scores,
            'reward_extra_infos': reward_results.get('extra_info', []),
            # Ultra-fast timing metrics
            'generation_time': timing_metrics.get('generation_time', 0),
            'total_tokens': timing_metrics.get('total_tokens', 0),
            'input_tokens': timing_metrics.get('input_tokens', 0),
            'output_tokens': timing_metrics.get('output_tokens', 0),
            'samples_per_sec': timing_metrics.get('samples_per_sec', 0),
            'tokens_per_sec': timing_metrics.get('tokens_per_sec', 0),
            'output_tokens_per_sec': timing_metrics.get('output_tokens_per_sec', 0)
        })
    ...
    def _save_batch_details(self, batch_metrics: Dict[str, Any], batch_idx: int, dataset_name: str):
        ...
        Ultra-fast mode skips this expensive I/O operation.
        ...
        Args:
            ...
            dataset_name: Name of the dataset being evaluated
        """
        # Skip expensive detailed results saving in ultra-fast mode
        if self.minimal_metrics:
            return
    ...
    def _aggregate_metrics(self, batch_metrics_list: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        # Add ultra-fast timing metrics aggregation
        timing_keys = ['generation_time', 'total_tokens', 'input_tokens', 'output_tokens', 
                      'samples_per_sec', 'tokens_per_sec', 'output_tokens_per_sec']
        
        for timing_key in timing_keys:
            timing_values = []
            for metrics in batch_metrics_list:
                if timing_key in metrics and isinstance(metrics[timing_key], (int, float)):
                    timing_values.append(metrics[timing_key])
            
            if timing_values:
                if timing_key == 'generation_time':
                    # Sum generation times for total evaluation time
                    aggregated[f'total_{timing_key}'] = np.sum(timing_values)
                    aggregated[f'avg_{timing_key}'] = np.mean(timing_values)
                elif '_per_sec' in timing_key:
                    # Average throughput metrics
                    aggregated[f'avg_{timing_key}'] = np.mean(timing_values)
                else:
                    # Sum token counts for totals
                    aggregated[f'total_{timing_key}'] = np.sum(timing_values)
                    aggregated[f'avg_{timing_key}'] = np.mean(timing_values)
...
def create_kg_evaluator(
    ...
    **kwargs
    ...
    Returns:
        Initialized RayKGEvaluatorLatency instance
    """
    import ray
