...
class RayPPOTrainer:
    ...
    def _create_dataloader(self, train_dataset, val_dataset, collate_fn, train_sampler):
        ...
        validation_shuffle = self.config.data.get("validation_shuffle", True)
        print(f"[DATALOADER DEBUG] validation_shuffle setting: {validation_shuffle}")
    ...
    def _validate(self):
        ...
        core_metrics = {
            'exact_match', 'retrieval_quality', 'total_score', 'num_turns',
        ...
        for data_source, var2metric2val in data_src2var2metric2val.items():
            for var_name, metric2val in var2metric2val.items():
                ...
                for metric_name, metric_val in metric2val.items():
                    ...
                    if var_name in ['exact_match', 'retrieval_quality']:
                        # These need bootstrap sampling metrics: mean@4 and mean@1
                        if metric_name in ['mean@4', 'mean@1']:
                            should_log = True
                    else:
    ...
    def _process_rollout_metrics(self, reward_extra_infos_dict: dict[str, list]) -> dict[str, float]:
        ...
        for var_name, values in reward_extra_infos_dict.items():
            ...
            # For exact_match and retrieval_quality: compute mean@4 and mean@1 if enough samples
            if var_name in ['exact_match', 'retrieval_quality'] and len(values) >= 4:
                # Bootstrap sampling for mean@4 and mean@1
                values_array = np.array(values)
                
                # mean@1: average of max values when sampling 1
                mean_1 = np.mean(values_array)  # For binary metrics, this is the same as overall mean
                
                # mean@4: average when sampling 4 (if we have enough samples)
                if len(values) >= 4:
                    # Simple approximation: sample 4 random values multiple times and take mean
                    n_bootstrap = min(100, len(values) // 4)
                    bootstrap_means = []
                    for _ in range(n_bootstrap):
                        sample = np.random.choice(values_array, size=4, replace=False)
                        bootstrap_means.append(np.mean(sample))
                    mean_4 = np.mean(bootstrap_means)
                else:
                    mean_4 = mean_val
                ...
                rollout_metrics[f"{metric_sec}/rollout/{var_name}/mean@1"] = mean_1
                rollout_metrics[f"{metric_sec}/rollout/{var_name}/mean@4"] = mean_4
            else:
    ...
    def init_workers(self):
        ...
        # Initialize generation manager for search if enabled
        if self.use_search_generation:
            from search_r1.llm_agent.generation import LLMGenerationManager, GenerationConfig
            gen_config = GenerationConfig(
                max_turns=self.search_config.get('max_turns', 3),
                max_start_length=self.config.data.get('max_start_length', 1024),
                ...
                search_url=self.search_config.get('search_url', 'http://127.0.0.1:8000/retrieve'),
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with _timer("step", timing_raw):
                    ...
                    with _timer("gen", timing_raw):
                        if not self.use_search_generation:
                            ...
                            first_input_ids = gen_batch.batch['input_ids'][:, -self.generation_manager.config.max_start_length:].clone().long()
                    ...
                    with _timer("old_log_prob", timing_raw):
                        ...
                        if (self.use_search_generation and 
                            ...
                            entropy_masks = batch.batch["loss_mask"]
                        else:
                ...
                # TODO: implement actual tflpo and theoretical tflop
    ...
    def _create_loss_mask(self, batch, metrics):
        ...
        if 'info_mask' not in batch.batch:
            raise ValueError("info_mask not found in batch when state_masking=true. "
                           "Make sure the LLM agent generation creates info_mask correctly.")
        
        response_length = batch.batch['responses'].shape[-1]
        response_mask = batch.batch['attention_mask'][:, -response_length:]
        
        # Use info_mask to create loss_mask for response tokens only
        info_mask = batch.batch['info_mask']
        loss_mask = info_mask[:, -response_length:]
        
        # Ensure loss_mask doesn't exceed response_mask bounds
        loss_mask = loss_mask * response_mask
        
        batch.batch['loss_mask'] = loss_mask
        
        # Debug logging: Track state token coverage
        total_response_tokens = response_mask.sum().item()
        total_state_tokens = loss_mask.sum().item()
        coverage_ratio = total_state_tokens / total_response_tokens if total_response_tokens > 0 else 0.0
        
        metrics.update({
            'state_masking/total_response_tokens': total_response_tokens,
            'state_masking/total_state_tokens': total_state_tokens,
            'state_masking/coverage_ratio': coverage_ratio,
        })
        
        print(f"[DEBUG] State masking - Response tokens: {total_response_tokens}, "
              f"State tokens: {total_state_tokens}, Coverage: {coverage_ratio:.3f}")
