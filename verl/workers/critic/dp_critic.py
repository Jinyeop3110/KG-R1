...
class DataParallelPPOCritic(BasePPOCritic):
    ...
    def _forward_micro_batch(self, micro_batch):
        ...
        with torch.autocast(device_type=self.device_name, dtype=torch.bfloat16):
            ...
            if self.use_remove_padding:
                ...
                # Ensure input_ids remain as integers (not affected by autocast)
                input_ids_rmpad_int = input_ids_rmpad.long()
                
                output = self.critic_module(
                    input_ids=input_ids_rmpad_int,
                    attention_mask=None,
    ...
    def update_critic(self, data: DataProto):
        ...
        # Note: We don't include loss_mask for critic - it should see all response tokens
        # State masking is only applied to policy loss calculation
        batch = data.select(batch_keys=select_keys).batch
        has_multi_modal_inputs = "multi_modal_inputs" in data.non_tensor_batch.keys()
        ...
        for epoch in range(self.config.ppo_epochs):
            for batch_idx, data in enumerate(dataloader):
                ...
                for data in micro_batches:
                    ...
                    # Always use attention_mask-based response_mask for critic loss calculation
                    # This ensures critic sees all response tokens, even when state_masking is enabled for policy
                    response_mask = attention_mask[:, -response_length:]
